...
class VeresiyeController extends Controller
{
    public function index()
    {
        ...
        $ExpenseObjective = DB::select("SELECT e.id as aid , e.* , o.* FROM expense_statements AS e LEFT JOIN objectives AS o ON e.expense_type_id = o.id where  1 = 2 " );
        ...
        FROM expense_statements
        WHERE YEAR(expense_date) = YEAR(DATE(NOW())) AND 1 = 2 and  MONTH(expense_date) = MONTH(DATE(NOW()))  AND company_id =  " . auth()->user()->company_id);
